...
> Did the resolution preserve both sides' intent, or just pick one?
...
```json
{"ts":"<UTC ISO 8601>","skill":"resolving-merge-conflicts","version":"<skill.version from CONFIG.yaml>","prompt":"<the user's original request>","outcome":<1-5>,"note":"<one line, especially when the outcome is not 4>","source":"llm","session":"<this session's id>","schema_version":1}
```
