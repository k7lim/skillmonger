...
> Could a human actually follow the wizard without reading its source?
...
```json
{"ts":"<UTC ISO 8601>","skill":"wizard","version":"<skill.version from CONFIG.yaml>","prompt":"<the user's original request>","outcome":<1-5>,"note":"<one line, especially when the outcome is not 4>","source":"llm","session":"<this session's id>","schema_version":1}
```
