...
Both axes should run independently, preferably as **parallel sub-agents** when the host supports them, so they don't pollute each other's context. Then this skill aggregates their findings.

The issue tracker should have been provided to you. If `docs/agents/issue-tracker.md` is missing, ask the user for the issue tracker workflow instead of assuming a specific setup command exists.
...
Use parallel sub-agents when available. If not available, run the Standards review and Spec review as separate passes and keep their notes isolated.
...
Self-assess: Did the review separate Standards from Spec, cite concrete files or requirements, and prioritize behavioral risks over summary?
