...
description: >-
  ...
  chart, graph, plot or dashboard; a static diagram or flowchart; a slide deck; a
  game with win conditions and scoring; a 3D scene needing WebGL, Three.js or real
...
`$SKILL_DIR` is wherever this skill is installed. Different harnesses expose it
differently, so resolve it rather than assuming:

```bash
SKILL_DIR="${CLAUDE_SKILL_DIR:-${AGENT_SKILL_DIR:-}}"
[ -n "$SKILL_DIR" ] || SKILL_DIR=$(dirname "$(find ~/.claude/skills ~/.codex/skills \
  ~/.config/opencode/skills ~/.pi/agent/skills -maxdepth 2 \
  -path '*isometric-explainer/SKILL.md' 2>/dev/null | head -1)")
```

If that comes up empty — likely under Codex or Gemini — just copy
`scripts/smoke.mjs` into the project next to `index.html` and run it from there.
It has no imports from the skill directory.
...
`scripts/check-prereqs.sh` reports what is actually installed before you start,
so you find out up front rather than at verification time.
...
```bash
scripts/evaluate.sh <project-dir>
```

It syntax-checks every JS file, serves the project, drives `scripts/smoke.mjs`
through every station, and scores the result. Use the JSON it returns.

If Playwright is missing, the evaluator caps the outcome at 3 and says so in the
