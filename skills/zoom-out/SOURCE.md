...
Upstream deleted this skill after we adopted it, so there is nothing left to sync. It is ours alone now. The `upstream:` block is kept for attribution under the MIT license, not for updates.
...
```bash
scripts/check-upstream.sh zoom-out
```
